...
function CalendarGrid({ periods, value, onPick }) {
  ...
  const [ym, setYm] = useState(() => {
    ...
    return { y: parts[0] || new Date().getFullYear(), m: parts[1] || new Date().getMonth() + 1 };
  });
  ...
  const submitPtTrial = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`${r.sent || 0}명에게 발송 완료` + (r.skipped ? ` (이미 발송된 ${r.skipped}명 제외)` : ''));
          ...
          <div className="cs-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="cs-target-box">
              {members.length === 0 ? <span className="cs-muted">대상 회원이 없습니다.</span>
                : members.map((m, idx) => (
                    <div key={`${m.username}-${idx}`} className="cs-target-row">
  ...
  return (
    <div className="cs-risklist">
      {riskList.map((m, idx) => {
        ...
        return (
          <div key={`${m.username}-${idx}`} className="cs-rmember">
...
function Report() {
  ...
  const [showRiskPop, setShowRiskPop] = useState(false);
